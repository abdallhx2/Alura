...
const Blob: React.FC<BlobProps> = ({ color = '#333333' }) => {
  ...
  useFrame((state) => {
    ...
    if (mesh.current) {
      ...
      // Smooth glow effect - slightly faster
      material.uniforms.u_time.value = 0.25 * clock.getElapsedTime();
      material.uniforms.u_intensity.value = MathUtils.lerp(
        material.uniforms.u_intensity.value,
        hover.current ? 0.3 : 0.45,
        0.012
      );
      // Gentle mouse tracking
      targetPosition.current.set(mouse.x * 0.2, mouse.y * 0.2, 0);
